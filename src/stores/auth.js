...
import { ref } from 'vue'
const url = 'https://new-2c9g.onrender.com/users'
...
export const useAuthStore = defineStore('auth', {
    ...
    actions: { // 상태 변경 - methods 역할
        ...
        //   })
        // },
        signIn(loginObj) { // 로그인 - 이메일, 패스워드 파라미터로 받아옴
          ...
          .then(res => {
            ...
            let userId = res.data.userId // 받아온 데이터 중 회원 id를 userId에 할당
        ...
        getUserInfo() { // 로그인 혹은 새로고침 시 로컬스토로지 저장된 토근을 헤더에 포함시켜 유저 정보를 요청
          ...
          if(token !== null && userId !== null) { // 토큰이 없거나 유저 id가 없을 시 (로그인 안된 상태)
            ...
            .then(res => {
              ...
              if(this.userInfo.email === 'admin@the-51.com') { // userInfo의 이메일이 어드민 이메일이라면
